"""Simple model for running tsunami inundation scenario in Purworejo, Indonesia, using ANUGA
...
import project_clip as project         # project.py consists of definition of file names and so on
...
if project.verbose: print 'Reading ASC from fixed_topobathy_clip.zip'
...
domain = anuga.create_domain_from_regions(project.bounding_polygon,
											boundary_tags={'east': [0],
														...
														'west': [2],
														'bottom_ocean': [3]},
											maximum_triangle_area=project.extent1_res,
...
#------------------------------------------------------------------------------
domain.set_name('pwrj_SA3_CF_' + project.scenario) # Name of sww file
...
# setting up friction value for each interior polygons
...
tmsfile = 'tmsfile3.txt'
print 'Converting time series file '+tmsfile+' to NetCDF TMS file format'
file_text = path_to_data_dir+tmsfile
anuga.file_conversion.file_conversion.timefile2netcdf(file_text, file_out = 'tmsfile.tms', quantity_names=None,
...
if project.scenario == 'fixed_wave':
	# Option 1: Huge 10.94m wave starting after 60 seconds and lasting 60 minutes.
	...
	# Option 3: simple time boundary condition if transmissive boundary create instabilty/timestepping too small exception
